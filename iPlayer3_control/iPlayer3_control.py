...
def connect(port=None):
    ...
    connection = serial.Serial(port, 9600, timeout=.01, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, bytesize=serial.EIGHTBITS)
    return(connection)

def trigger_show(connection, show_number, await_response=False):

    # Send a serial command over the given connection to trigger playback of the
    # given show.
...
def set_absolute_intensity(connection, val, await_response=False):
    ...
    # Send a serial command over the given connection to set the overall
    # intensity. val should be in the range [0:1] or [1:100]
...
def fade_to_black(connection):
    ...
    # Function to transition from the current brightness to zero in a smooth
    # fade
...
def fade_to_bright(connection):
    ...
    # Function to transition from the current brightness to max in a smooth
    # fade
